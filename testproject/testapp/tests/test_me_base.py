...
class TestBaseMeAPIView:
    def test_get_queryset_with_custom_queryset(self, user):
        view = BaseMeAPIView()
        view.queryset = User.objects.all()  # Set custom queryset
...
def test_get_queryset_returns_only_current_user(user, api_client):
    """
    get_queryset must be usable; it previously called .objects on a QuerySet.
    """
    from rest_framework.test import APIRequestFactory

    from testapp.factories import UserFactory

    UserFactory.create(username="other", email="other@example.com")
