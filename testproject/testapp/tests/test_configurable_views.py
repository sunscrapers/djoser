...
def test_every_configured_view_key_is_used(settings):
    """
    A key in VIEWS that no URLConf reads is a silent no-op for anyone overriding it.
    """
    import pathlib
    import re
    ...
    defined = set(default_settings["VIEWS"].keys())

    read = set()
    for module in pathlib.Path("djoser/urls").glob("*.py"):
        source = module.read_text()
        # {"GET": "user_list", ...} method -> config key maps
        read |= set(re.findall(r'"(?:GET|POST|PUT|PATCH|DELETE)":\s*"(\w+)"', source))
        # getattr(settings.VIEWS, "activation", None)
        read |= set(re.findall(r'settings\.VIEWS,\s*"(\w+)"', source))
        # settings.VIEWS.token_create
        read |= set(re.findall(r"settings\.VIEWS\.(\w+)", source))

    assert (
        defined - read == set()
    ), f"VIEWS keys never read by any URLConf: {defined - read}"
